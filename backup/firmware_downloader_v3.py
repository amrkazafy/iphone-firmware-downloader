...
def wait_for_idm(filepath, expected_size, timeout_minutes=180):
    last_size = 0
    stall_count = 0
    start = time.time()
    ...
    while time.time() - start < timeout_minutes * 60:
        ...
        if filepath.exists():
            ...
            tmp_files = list(filepath.parent.glob(f"{filepath.name}*"))
            if not tmp_files:
...
def download_with_idm(url, output_path, expected_size):
    ...
    try:
        ...
        pass
    return wait_for_idm(output_path, expected_size)
...
def process_device(device):
    ...
    try:
        ...
        return {"identifier": identifier, "name": device_name, "status": "error", "error": f"API: {e}"}

    firmwares = data.get("firmwares", [])
    latest = get_latest_firmware(firmwares)

    if not latest:
        return {"identifier": identifier, "name": device_name, "status": "skipped", "error": "No firmwares"}
    ...
    if url in downloaded_urls:
        return {"identifier": identifier, "name": device_name, "version": version, "build": build, "status": "exists", "size": filesize}
    ...
    if not success:
        return {"identifier": identifier, "name": device_name, "version": version, "status": "error", "error": msg}

    valid, msg = verify_file(output_path, sha1=sha1, expected_size=filesize)
    status = "verified" if valid else "corrupt"
    if valid:
        downloaded_urls.add(url)

    return {
        "identifier": identifier, "name": device_name,
        "version": version, "build": build,
        "status": status, "verification": msg, "size": filesize
    }
...
def main():
    ...
    for i, dev in enumerate(iphones, 1):
        ...
        icon = {"exists": "[=]", "verified": "[+]", "error": "[x]", "corrupt": "[!]", "skipped": "[-]"}
        i_icon = icon.get(result["status"], "[?]")
        ver = result.get("version", "?")
        msg = result.get("verification", result.get("error", ""))
    ...
    print(f"  Errors (URL dead):      {errors}")
    ...
    if errors > 0 or corrupt > 0:
        print("\n  Devices with issues:")
        for r in results:
            if r["status"] in ("error", "corrupt"):
